...
function ActionButton({ label, href, onClick, primary = false }) {
  ...
  const style = primary
    ? {
        ...commonStyle,
        background: "linear-gradient(135deg, #E58D19 0%, #D97706 100%)",
        border: "none",
        color: "#fff",
      }
    : {
...
export default function DuelHeroCardV3Clean({
  ...
  const fonsecaPct = probability?.fonseca ?? null;
  const opponentPct = probability?.opponent ?? null;
    ...
    >
      ...
      <div style={{ padding: "18px 18px 0" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontSize: 10, fontWeight: 800, color: "#4FC3F7", fontFamily: SANS, textTransform: "uppercase", letterSpacing: "0.08em" }}>
              {statusLabel}
            </span>
      ...
      <div style={{ padding: "14px 18px 0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          ...
          <InfoChip label="Quadra confirmada" value={courtLabel || "A definir"} icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4FC3F7" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0118 0z" /><circle cx="12" cy="10" r="3" /></svg>} />
      ...
      {countdownLabel ? (
        <div style={{ padding: "12px 18px 0" }}>
          <div style={{ background: "rgba(79,195,247,0.10)", border: "1px solid rgba(79,195,247,0.16)", borderRadius: 14, padding: "12px 14px", textAlign: "center" }}>
            <div style={{ fontSize: 11, fontWeight: 800, color: "#4FC3F7", fontFamily: SANS, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 2 }}>Falta para o jogo</div>
      ...
      <div style={{ padding: "12px 18px 0" }}>
        <div style={{ display: "flex", gap: 10 }}>
          <ActionButton label={primaryActionLabel} href={primaryActionHref} onClick={primaryActionOnClick} primary />
          <ActionButton label={secondaryActionLabel} href={secondaryActionHref} onClick={secondaryActionOnClick} />
        </div>
      </div>

      {fonsecaPct !== null && opponentPct !== null ? (
        <div style={{ padding: "16px 18px 0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <span style={{ fontSize: 20, fontWeight: 900, color: fonsecaPct >= opponentPct ? GREEN : "rgba(255,255,255,0.30)", fontFamily: SANS }}>{`${Math.round(fonsecaPct)}%`}</span>
            <span style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.22)", fontFamily: SANS, textTransform: "uppercase", letterSpacing: "0.06em" }}>Probabilidade de vitória</span>
            <span style={{ fontSize: 20, fontWeight: 900, color: opponentPct > fonsecaPct ? "#ef4444" : "rgba(255,255,255,0.30)", fontFamily: SANS }}>{`${Math.round(opponentPct)}%`}</span>
          </div>
          <div style={{ display: "flex", gap: 2, height: 6, borderRadius: 3, overflow: "hidden" }}>
            <div style={{ width: `${Math.round(fonsecaPct)}%`, background: `linear-gradient(90deg, ${GREEN}, #22c55e)` }} />
            <div style={{ width: `${Math.round(opponentPct)}%`, background: "linear-gradient(90deg, #ef4444, #f87171)" }} />
          </div>
        </div>
      ) : null}
