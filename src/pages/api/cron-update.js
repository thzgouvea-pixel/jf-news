...
var TOURNAMENT_CURIOSITIES={"monte carlo":[{icon:"🏔️",text:"Jogado num penhasco com vista pro Mediterrâneo"},{icon:"👑",text:"Nadal venceu 11 vezes — recorde absoluto"},{icon:"🎾",text:"Primeiro Masters 1000 de saibro da temporada"},{icon:"🇲🇨",text:"O Country Club tem apenas 1 quadra central com 10.200 lugares"},{icon:"📜",text:"Único Masters que não é obrigatório no calendário ATP"}],"roland garros":[{icon:"🏟️",text:"A quadra Philippe-Chatrier tem teto retrátil desde 2020"},{icon:"👑",text:"Nadal venceu 14 vezes — o maior domínio da história do esporte"},{icon:"🧱",text:"O saibro é feito de pedra calcária triturada de Millau"},{icon:"📜",text:"Nomeado em homenagem a um aviador da 1ª Guerra Mundial"},{icon:"🇧🇷",text:"Guga é o único brasileiro campeão (3x)"}],"wimbledon":[{icon:"🍓",text:"Cerca de 28 mil kg de morangos são consumidos durante o torneio"},{icon:"🌱",text:"A grama é cortada a exatos 8mm de altura"},{icon:"👔",text:"Código de vestimenta obrigatório: quase tudo branco"},{icon:"📜",text:"O torneio mais antigo do mundo: desde 1877"},{icon:"🏆",text:"Federer é o maior campeão com 8 títulos"}],"australian open":[{icon:"🌡️",text:"Temperaturas podem passar dos 40°C — há regra de calor extremo"},{icon:"🏟️",text:"Rod Laver Arena tem teto retrátil desde 1988"},{icon:"🇧🇷",text:"João Fonseca estreou em Grand Slam aqui em 2025"},{icon:"🦘",text:"É o único Grand Slam no hemisfério sul"},{icon:"👑",text:"Djokovic é o maior campeão com 10 títulos"}],"us open":[{icon:"✈️",text:"A quadra central fica na rota de aviões do aeroporto LaGuardia"},{icon:"🏟️",text:"Arthur Ashe Stadium: maior quadra de tênis do mundo (23.771 lugares)"},{icon:"🌙",text:"Famoso pelas sessões noturnas sob as luzes de Nova York"},{icon:"💰",text:"A maior premiação de todos os Grand Slams"},{icon:"🎵",text:"O barulho da torcida é tão alto que incomoda até veteranos"}],"madrid":[{icon:"🏔️",text:"Altitude de 650m faz a bola viajar mais rápido"},{icon:"🧱",text:"Já foi jogado em saibro azul em 2012 — gerou polêmica"},{icon:"🏟️",text:"Caja Mágica: complexo futurista com 3 quadras com teto retrátil"},{icon:"👑",text:"Nadal venceu 5 vezes no saibro de Madrid"},{icon:"♀️",text:"Um dos poucos Masters que também tem torneio feminino"}],"roma":[{icon:"🏛️",text:"Foro Itálico: arena construída na era Mussolini, nos anos 1930"},{icon:"🎨",text:"As estátuas de mármore ao redor das quadras são de 1928"},{icon:"👑",text:"Nadal venceu 10 vezes — recorde do torneio"},{icon:"🍕",text:"Apelidado de 'internazionali' pelos italianos"},{icon:"📜",text:"Existe desde 1930 — um dos mais antigos do circuito"}],"indian wells":[{icon:"🏜️",text:"No meio do deserto da Califórnia, com 350 dias de sol por ano"},{icon:"🏟️",text:"Stadium 1 tem 16.100 lugares — 2º maior do mundo"},{icon:"🌴",text:"Apelidado de 'o 5º Grand Slam' pela qualidade"},{icon:"💰",text:"Um dos torneios mais ricos fora dos Slams"},{icon:"🌅",text:"Jogos ao pôr do sol com montanhas ao fundo"}],"miami":[{icon:"🏟️",text:"Jogado no Hard Rock Stadium — casa do Miami Dolphins (NFL)"},{icon:"🌴",text:"Mudou-se de Key Biscayne para o estádio de futebol em 2019"},{icon:"🇧🇷",text:"Guga chegou à final em 2001"},{icon:"💨",text:"Vento forte vindo do mar é um desafio constante"},{icon:"🎉",text:"Famoso pela atmosfera festiva — mistura de esporte e entretenimento"}],"barcelona":[{icon:"🧱",text:"Real Club de Tenis Barcelona: fundado em 1899"},{icon:"👑",text:"Nadal venceu 12 vezes — domínio quase total"},{icon:"🏟️",text:"Quadra central com vista para as colinas de Barcelona"},{icon:"📜",text:"Um dos ATP 500 mais tradicionais do saibro"},{icon:"🇪🇸",text:"Ponto alto da temporada de saibro espanhola"}],"rio open":[{icon:"🇧🇷",text:"Único ATP 500 da América do Sul"},{icon:"🏟️",text:"Jogado no Jockey Club Brasileiro, em frente à Lagoa"},{icon:"🎉",text:"Atmosfera única: samba, calor e torcida apaixonada"},{icon:"👑",text:"Guga nunca jogou o Rio Open — o torneio começou em 2014"},{icon:"🏆",text:"Primeiro título ATP do João Fonseca!"}]};

async function fetchTournamentFacts(tournamentName,log){
  if(!tournamentName){log.push("tournament: no name");return;}

  // Check curated curiosities first — always preferred
  var tL=tournamentName.toLowerCase();
  var curated=null;
  for(var ck in TOURNAMENT_CURIOSITIES){if(tL.includes(ck)){curated=TOURNAMENT_CURIOSITIES[ck];break;}}

  try{var cached=await kv.get("fn:tournamentFacts");if(cached){var p=typeof cached==="string"?JSON.parse(cached):cached;var same=p.name&&tournamentName.toLowerCase().includes(p.name.toLowerCase().split(" ")[0]);var clean=true;if(p.facts){for(var fi=0;fi<p.facts.length;fi++){if((p.facts[fi].text||"").indexOf("[[")!==-1||(p.facts[fi].text||"").indexOf("{{")!==-1){clean=false;break;}}}if(same&&clean&&(p.source==="curated"||!curated)){log.push("tournament: cache ok ("+p.name+")");return;}}}catch(e){}

  if(curated){
    await kv.set("fn:tournamentFacts",JSON.stringify({name:tournamentName,facts:curated,source:"curated",updatedAt:new Date().toISOString()}),{ex:86400*14});
    log.push("tournament: "+curated.length+" curated facts");
    return;
  }

  // Fallback: Wikipedia parsing for unknown tournaments
  var wikiPage=null;for(var key in TOURNAMENT_WIKI_MAP){if(tL.includes(key)){wikiPage=TOURNAMENT_WIKI_MAP[key];break;}}
  ...
  try{
    ...
    log.push("tournament: "+facts.length+" wiki facts");
