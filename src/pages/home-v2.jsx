...
import NextDuelCard from "../components/home/NextDuelCard";
...
export default function HomeV2() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff" }}>
      <Head>
        <title>Fonseca News — Home V2</title>
        <meta name="description" content="Preview da home refatorada do Fonseca News." />
      </Head>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <span style={{ fontSize: 10, color: DIM, fontFamily: SANS, display: "block", marginTop: 2 }}>
                Preview da home refatorada{lastUpdate ? ` · ${formatTimeAgo(lastUpdate)}` : ""}
              </span>
            </div>
          </div>
          <a href="/" style={{ fontSize: 12, fontWeight: 700, color: GREEN, fontFamily: SANS, textDecoration: "none" }}>Voltar</a>
        </div>
      </header>
      ...
      <main style={{ maxWidth: 640, margin: "0 auto", padding: "0 12px 36px" }}>
        <section style={{ padding: "8px 0 0" }}>
          <NextDuelCard
            ...
            onOppClick={opponentProfile ? () => setShowOppPopup(true) : null}
            winProb={winProb}
            oppProfile={opponentProfile}
            liveData={liveMatch}
            tournamentFacts={tournamentFacts?.facts || null}
          />
        </section>
        ...
        <section style={{ padding: "20px 0 0" }}>
          ...
          {highlightVideo?.videoId ? (
            <MatchCarousel
              ...
              playerRanking={displayPlayer?.ranking}
          ...
          {!loading ? (
            <div>
              {displayNews.slice(0, 8).map((item, index) => (
                <NewsCard key={`${item.title}-${index}`} item={item} index={index} allLikes={allLikes} noBorder={index === Math.min(displayNews.length, 8) - 1} />
              ))}
            </div>
          ) : null}
        </section>
        ...
        <section style={{ padding: "24px 0 0" }}>
          <div style={{ background: BG_ALT, borderRadius: 14, padding: "16px 18px", border: `1px solid ${BORDER}` }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {[
                "NextDuelCard já está desacoplado da home principal",
                "MatchCarousel e PlayerBlock já funcionam como módulos independentes",
                "NewsCard saiu do arquivo monolítico",
                "Modal compartilhado e utilitários base já estão extraídos",
              ].map((text, index) => (
                <div key={index} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                  <span style={{ color: GREEN, fontSize: 12, fontWeight: 700, fontFamily: SANS, marginTop: 1 }}>•</span>
                  <span style={{ fontSize: 12, color: SUB, fontFamily: SANS, lineHeight: 1.5 }}>{text}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {showOppPopup && opponentProfile ? (
        <Modal title={opponentProfile.name || "Oponente"} subtitle="Preview do modal refatorado" onClose={() => setShowOppPopup(false)} maxWidth={420}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ["País", opponentProfile.country || "—"],
              ["Ranking", opponentProfile.ranking ? `#${opponentProfile.ranking}` : "—"],
              ["Idade", opponentProfile.age || "—"],
              ["Mão", opponentProfile.hand || "—"],
            ].map(([label, value]) => (
              <div key={label} style={{ padding: "12px", borderRadius: 12, background: BG_ALT, border: `1px solid ${BORDER}` }}>
                <span style={{ fontSize: 10, color: DIM, fontFamily: SANS, textTransform: "uppercase", letterSpacing: "0.05em", display: "block", marginBottom: 4 }}>{label}</span>
                <span style={{ fontSize: 14, color: TEXT, fontFamily: SANS, fontWeight: 700 }}>{value}</span>
              </div>
            ))}
          </div>
          {opponentProfile.style ? (
            <p style={{ margin: "14px 0 0", fontSize: 13, color: SUB, fontFamily: SANS, lineHeight: 1.6 }}>{opponentProfile.style}</p>
          ) : null}
        </Modal>
      ) : null}
    </div>
