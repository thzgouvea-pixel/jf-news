...
function countdown(date) {
  ...
  const out = [];
  if (days > 0) out.push(`${days} ${days === 1 ? "dia" : "dias"}`);
  if (hours > 0) out.push(`${hours} ${hours === 1 ? "hora" : "horas"}`);
  if (minutes > 0 && days === 0) out.push(`${minutes} ${minutes === 1 ? "minuto" : "minutos"}`);
  return out.length ? out.join(" e ") : "Menos de 1 minuto";
}

function imgFor(name) {
  if (!name) return null;
  const k = Object.keys(PLAYER_IMG).find((x) => name.includes(x));
  return k ? PLAYER_IMG[k] : null;
}
...
function Player({ image, name, meta, accent }) {
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ width: 104, height: 104, borderRadius: "50%", overflow: "hidden", margin: "0 auto 10px", background: "rgba(255,255,255,0.08)", border: accent ? `3px solid ${C.green}55` : "3px solid rgba(255,255,255,0.16)", boxShadow: accent ? "0 0 0 7px rgba(0,168,89,0.08)" : "0 0 0 7px rgba(255,255,255,0.05)" }}>
        {image ? <img src={image} alt={name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.45)", fontFamily: SANS, fontSize: 32, fontWeight: 800 }}>{name?.[0] || "?"}</div>}
      </div>
      <div style={{ fontSize: 17, fontWeight: 800, color: "#fff", fontFamily: SERIF }}>{name}</div>
...
function StatPill({ label, value, accent }) {
  return (
    <div style={{ textAlign: "center", background: C.card, border: `1px solid ${C.line}`, borderRadius: 18, padding: "14px 10px", boxShadow: "0 8px 24px rgba(15,23,42,0.04)" }}>
      <div style={{ fontSize: 18, fontWeight: 900, color: accent || C.text, fontFamily: SANS }}>{value}</div>
      <div style={{ fontSize: 9, fontWeight: 800, color: C.dim, fontFamily: SANS, marginTop: 5, textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</div>
    </div>
  );
}
...
export default function HomeMatchdayV2() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg }}>
      <Head>
        ...
        <meta name="description" content="Homepage matchday v2" />
      </Head>
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "12px 12px 32px" }}>
        <section style={{ paddingBottom: 24 }}>
          <section style={{ background: `linear-gradient(160deg, ${C.dark1} 0%, ${C.dark2} 54%, ${C.dark3} 100%)`, borderRadius: 28, overflow: "hidden", boxShadow: "0 22px 50px rgba(7,12,22,0.18)" }}>
            <div style={{ padding: "18px 18px 14px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <h1 style={{ margin: "0 0 4px", fontSize: 18, fontWeight: 700, color: "rgba(255,255,255,0.78)", fontFamily: SANS }}>{nextMatch?.tournament_category || "ATP Tour"}</h1>
            ...
            <div style={{ padding: 18 }}>
              ...
              <div style={{ display: "grid", gap: 10, marginBottom: 12 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  ...
                  <MetaBox label="Horário em Brasília" value={fmtTime(nextMatch?.date)} />
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <MetaBox label="Quadra confirmada" value={nextMatch?.court || "A definir"} />
                  <MetaBox label="Onde assistir" value="ESPN 2 · Disney+" />
                </div>
              </div>
              ...
              <div style={{ borderRadius: 18, padding: "14px 16px", textAlign: "center", marginBottom: 12, background: "linear-gradient(180deg, rgba(79,195,247,0.12), rgba(79,195,247,0.08))", border: "1px solid rgba(79,195,247,0.18)" }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: C.blue, fontFamily: SANS, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>Falta para o jogo</div>
                <div style={{ fontSize: 18, fontWeight: 900, color: "#fff", fontFamily: SANS }}>{countdown(nextMatch?.date)}</div>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                <a href="https://www.disneyplus.com" target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", textAlign: "center", padding: "16px 12px", borderRadius: 16, color: "#fff", fontSize: 14, fontWeight: 800, fontFamily: SANS, background: "linear-gradient(135deg,#F59E0B 0%, #EA8A00 100%)" }}>Onde assistir</a>
        ...
        <section style={{ paddingBottom: 24 }}>
          <Label>O que acabou de acontecer</Label>
          <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 24, padding: 22, boxShadow: "0 12px 32px rgba(15,23,42,0.05)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.2fr .8fr", gap: 12 }}>
              ...
              <div style={{ background: C.bg, border: `1px solid ${C.line}`, borderRadius: 18, padding: 16 }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{form.map((m, i) => <span key={i} style={{ width: 28, height: 28, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", background: m.result === "V" ? `${C.green}14` : "#FEE2E2", color: m.result === "V" ? C.green : "#DC2626", fontSize: 11, fontWeight: 800, fontFamily: SANS }}>{m.result}</span>)}</div>
              </div>
            </div>
          </div>
        </section>
